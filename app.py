...
# ========================================================
# SQL 정제, 대소문자 중복 제거 및 Google Sheets API v4 파이프라인
# ========================================================

def sanitize_sql(sql_str):
    """생성된 SQL 문장의 백틱, 미완성 구문, 잘린 괄호/따옴표를 정제하는 함수"""
    # 정규표현식(re.sub)을 사용하여 백틱 문자로 인한 SyntaxError 예방
    sql = re.sub(r'```(?:sql)?', '', sql_str).strip()
    if sql.endswith(";"):
        sql = sql[:-1].strip()
...
def find_and_set_header(df_raw):
    """최적의 헤더 탐색 및 SQLite 대소문자 중복 컬럼 완전 차단"""
    if df_raw.empty:
        return df_raw
    ...
    for col in new_cols:
        ...
        if col_lower in seen_lower:
            ...
            unique_cols.append(f"{col}_{seen_lower[col_lower]}")
        else:
...
def standardize_columns_for_domain(df, file_name, tab_name):
    """
    Sales(수출)와 Activation(실판매) 컬럼 인플레이스 표준화 및 대소문자 중복 제거
    """
    if df.empty:
        return df, "UNKNOWN"
    ...
    # 1. Activation 파일 전처리: 수량(Qty = 1) 가상 열 생성 및 날짜/국가 표준화
    if is_activation:
        if 'qty' not in cols_map:
            df['Qty'] = 1
            cols_map['qty'] = 'Qty'
        
        if 'created_year' in cols_map and 'year' not in cols_map:
            rename_dict[cols_map['created_year']] = 'Year'
            cols_map['year'] = 'Year'
        if 'created_month' in cols_map and 'month' not in cols_map:
            rename_dict[cols_map['created_month']] = 'Month'
            cols_map['month'] = 'Month'
            
        if 'country_renamed' in cols_map and 'country' not in cols_map:
            rename_dict[cols_map['country_renamed']] = 'country'
            cols_map['country'] = 'country'

    # 2. 공통 국가 컬럼 인플레이스 표준화 (Country -> country)
    if 'country' not in df.columns:
        for c in df.columns:
            if str(c).lower() in ['country', 'country_renamed']:
                rename_dict[c] = 'country'
                break

    if rename_dict:
        df = df.rename(columns=rename_dict)

    # 3. SQLite 대소문자 무관 중복 컬럼명 안전 처리 (duplicate column name 에러 방지)
    seen_lower = {}
    unique_cols = []
